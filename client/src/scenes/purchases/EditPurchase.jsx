...
import { fetchPurchase, updatePurchase } from "../../api/purchase";
...
const EditPurchase = () => {
  ...
  const [purchaseQuery, supplierQuery] = useQueries({
    queries: [
      {
  ...
  const formik = useFormik({
    initialValues: {
      ref: purchaseQuery.isLoading ? "" : purchaseQuery.data.ref,
      supplierId: purchaseQuery.isLoading
        ? ""
        : purchaseQuery.data.supplierId._id,
      pdate: purchaseQuery.isLoading
        ? ""
        : moment(purchaseQuery.data.pdate).format("YYYY-MM-DD"),
      discount: purchaseQuery.isLoading ? "" : purchaseQuery.data.discount,
      tax: purchaseQuery.isLoading ? "" : purchaseQuery.data.tax,
      shipping: purchaseQuery.isLoading ? "" : purchaseQuery.data.shipping,
      status: purchaseQuery.isLoading ? "" : purchaseQuery.data.status,
      note: purchaseQuery.isLoading ? "" : purchaseQuery.data.note,
    },
    validationSchema,
  ...
  if (purchaseQuery.isError)
    return toast.error(purchaseQuery.error.response.data.error);
  if (supplierQuery.isError)
    return toast.error(supplierQuery.error.response.data.error);
            ...
            <Grid2 size={4}>
              ...
              >
                ...
                {!supplierQuery.isLoading ? (
                  supplierQuery.data.map((sup, index) => (
              ...
              {purchaseQuery.isLoading ? (
                ...
                <TableContainer component={Paper}>
                  <Table aria-label="simple table">
                    ...
                    <TableBody>
                      {purchaseQuery.data.items.map((item, index) => (
                        <TableRow
                          key={index}
                          sx={{
